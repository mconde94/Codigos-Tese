...
class Simulation:  
    ...
    def __init__(self,tipo,numero,topologia,c11,c22):
        ...
        self.ConstantC1=c11
        self.ConstantC2=c22
        self.Grid=Network(numero,tipo,topologia)
    ...
    def IsingAntBasedSimulation(self):
        ...
        for t in range (1,self.Time):
                epsilont[t]=self.ConstantRhoOut*epsilont[t-1] + self.ConstantSigma1*np.random.randn() #shocks in output equation (demand shock)
                etat[t]= self.ConstantRhoInf*etat[t-1] + self.ConstantSigma2*np.random.randn() #shocks in inflation equation (supply shock)
                ut[t]=self.ConstantRhoTayl*ut[t-1] + self.ConstantSigma3*np.random.randn() #shocks in Taylor rule (interest rate shock)
                epsilon=epsilont[t]
                eta=etat[t]
                u=ut[t]
                ...
                D=B.dot(forecast).reshape((-1,1)) + C.dot(lag).reshape((-1,1)) + rlag*smooth.reshape((-1,1)) + shocks.reshape((-1,1))
                X=linalg.solve(A,D)
                p[t]= X[0]
                y[t]= X[1]
                ...
                alfay=self.ConstantRhoBH*alfayt[t-1]+(1-self.ConstantRhoBH)*math.exp(self.Gamma*CRy[t])/(math.exp(self.Gamma * CRy[t]) + math.exp(self.Gamma * FRy[t]))
                ExtMagIn=2*alfay-1
                self.Grid.IsingInteraction(self.Gamma,ExtMagIn)
    ...
    def DeGrauweSimulation(self):
        ...
        for t in range (1,self.Time):
                ...
                plagt[t]=p[t-1]
                ylagt[t]=y[t-1]
                inter = self.Grid.DeGrauweInteraction(self.ConstantRho, eyfun, yt, self.ConstantRhoBH, self.Gamma)
                alfay=inter[0]
                eychar=inter[1]
                alfayt[t] = alfay
                if eychar>0:
