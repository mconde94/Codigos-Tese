...
class Network:
    ...
    def SumOverNeighbours(self,index,OldGrid):
        ...
        if len(nearest)>0:
            ...
            Soma=0
        return Soma
    def AgentsInovation(self):
        for i in range(0,self.NumberOfAgents):
            self.List[i].Inovation()
    def IsingInteraction(self,Lambda,extMag):
        #inovate
        self.AgentsInovation()
        #interaction
        valor=0
        OldStates=self.List
        NewStates=self.List
        for i in range (0,self.NumberOfAgents):
            ...
            Magnetization=(extMag+self.SelfConversion)*OldStates[i].IsingExpectative+Soma
            ...
            self.List[i].PastIsingExpectative=np.append(self.List[i].PastIsingExpectative,self.List[i].IsingExpectative)
        self.List=NewStates
        valor=sum(a.IsingExpectative for a in self.List)/len(self.List)
        self.Expectative=valor
    def DeGrauweInteraction(self,ro,eyfun,yt,roBH,gamma):
        # inovate all agents
        self.AgentsInovation()
        # agents analyze the situation
        for i in range(0, self.NumberOfAgents):
            self.List[i].ClassificationDeGrauwe(ro,eyfun,yt,roBH,gamma)
        # agents interact
        ...
        for i in range(0, self.NumberOfAgents):
            ...
            self.List[i].PastOutputGapExpectative=np.append(self.List[i].PastOutputGapExpectative,self.List[i].OutputGapExpectative)
