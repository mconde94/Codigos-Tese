...
coordenadas=divmod(medias.argmin(), medias.shape[1])


nhist=500
valoresHist=np.zeros((nhist,1))
for i in range(nhist):
    sim = OptimalPolicyHelper(coordenadas)
    valoresHist[i]=sim.StdY


fig = plt.figure(1)
MakeAnHistogram(valoresHist,'Distribution of the Output Gap variance',20)

sim = OptimalPolicyHelper(coordenadas)
sim.TimeSeriesGraph(1)
sim.FrequencyGraphOfTimeSeries(6)
...
sio.savemat(nome, mdict={'medias': medias, 'c1': c1, 'c2': c2, 'coordenadas':coordenadas})
