...
def ConnectedEuropeanNetwork():
    numAgentes = np.genfromtxt('Europa.csv', delimiter=',')
    totalAgentes = int(np.sum(numAgentes) + 1)
    redeFinal = StarNetwork(totalAgentes)
    i = 1
    r = int(numAgentes[0])
    j = 1 + r
    redeFinal[i:j, i:j] = CircularNetwork(r, 1)
    for z in range(1, np.size(numAgentes)):
        if r > 1:
            redeFinal[i:j, i:j] = CircularNetwork(r, 1)
            r = int(numAgentes[z])
        else:
...
def ConnectedAmericanNetwork():
    numAgentes = np.genfromtxt('America.csv', delimiter=',')
    totalAgentes = int(np.sum(numAgentes) + 1)
    redeFinal = StarNetwork(totalAgentes)
    i = 1
    r = int(numAgentes[0])
...
def SeparatedEuropeanNetwork():
    numAgentes = np.genfromtxt('Europa.csv', delimiter=',')
    totalAgentes = int(np.sum(numAgentes) + 1)
    redeFinal = SelfNetwork(totalAgentes)
    i = 1
    r = int(numAgentes[0])
    j = 1 + r
    redeFinal[i:j, i:j] = CircularNetwork(r, 1)
...
def ConnectedBrazilNetwork():
    numAgentes = np.genfromtxt('Brasil.csv', delimiter=',')
    totalAgentes = int(np.sum(numAgentes) + 1)
    redeFinal = StarNetwork(totalAgentes)
    i = 1
    r = int(numAgentes[0])
...
def ConnectedMexicanNetwork():
    numAgentes = np.genfromtxt('Mexico.csv', delimiter=',')
    totalAgentes = int(np.sum(numAgentes) + 1)
    redeFinal = StarNetwork(totalAgentes)
    i = 1
    r = int(numAgentes[0])
